...
namespace AlgoNet.Clustering.Generic
{
    ...
    public static class KMeans
    {
        ...
        public static KMeansCluster<T, TShape, TWeight, TDistance>[] Cluster<T, TShape, TWeight, TDistance>(
            ...
            where TDistance : IFloatingPoint<TDistance>
        {
            ...
            KMeansCluster<T, TShape, TWeight, TDistance>[] clusters = Split<T, TShape, TWeight, TDistance>(points, config.ClusterCount);
            ...
            while (changed)
            {
                ...
                {
                    KMeansCluster<T, TShape, TWeight, TDistance> cluster = clusters[i];
        ...
        private static int FindNearestClusterIndex<T, TShape, TWeight, TDistance>(
            T point,
            ...
            where TDistance : IFloatingPoint<TDistance>
        {
            ...
            {
                TDistance distance = shape.FindDistanceSquared(point, clusters[k].Centroid);
        ...
        private static KMeansCluster<T, TShape, TWeight, TDistance>[] Split<T, TShape, TWeight, TDistance>(
            ReadOnlySpan<T> points,
            int clusterCount)
            where T : unmanaged
            where TShape : struct, IGeometricPoint<T, TWeight, TDistance>
            ...
            where TDistance : IFloatingPoint<TDistance>
        {
            ...
            KMeansCluster<T, TShape, TWeight, TDistance>[] clusters = new KMeansCluster<T, TShape, TWeight, TDistance>[clusterCount];
            int subSize = points.Length / clusterCount;

            int iterationPos = 0;
            for (int i = 0; i < clusterCount; i++)
            {
                KMeansCluster<T, TShape, TWeight, TDistance> currentCluster = new KMeansCluster<T, TShape, TWeight, TDistance>();
